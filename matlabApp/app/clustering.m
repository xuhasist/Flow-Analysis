clearvars -except flow_table_temp_f host_ip_temp_f sw_struct_temp_f link_if_temp_f link_temp_f link_struct_temp_f sw_vector_temp_f g_
...
%{
flow_table_temp_f = flow_table;
host_ip_temp_f = host_ip;
sw_struct_temp_f = sw_struct;
link_if_temp_f = link_if;
link_temp_f = link;
link_struct_temp_f = link_struct;
sw_vector_temp_f = sw_vector;
%}
...
for i = 1:size(flow_table, 1)
    ...
    if doHierarchical
        ...
        sw_struct = removeAllFlowEntry(sw_struct, flow_start_datetime);
    end
    ...
    if has_hierarchy
        ...
        for j = 1:length(sw_list) - 1
            ...
            [flow_entry_temp, flow_src_ip, flow_dst_ip] = setFlowEntry(prefix_list(j), flow_entry, flow_table, i);
            ...
            round = j;
            [final_path_temp, sw_struct, link, finish] = processPkt_mod(g, link, link_bwd_unit, link_if, host_ip, sw_struct, ...
                preLower, link_struct, flow_entry_temp, flow, final_path_temp, flow_start_datetime, ...
                first_edge_sw, second_edge_sw, round, dst_name, flow_src_ip, flow_dst_ip);
        ...
        prefix_length = flow_table.prefix(i);
        [flow_entry, flow_src_ip, flow_dst_ip] = setFlowEntry(prefix_length, flow_entry, flow_table, i);
        rows = strcmp(link_if.Src_Node, src_name);
        flow_entry.input = link_if{rows, {'Dst_Inf'}};
    
        flow.dst_name = dst_edge_sw;
        round = 3;
        [final_path, sw_struct, link, finish] = processPkt_mod(g, link, link_bwd_unit, link_if, host_ip, sw_struct, ...
            preLower, link_struct, flow_entry, flow, final_path, flow_start_datetime, ...
            src_edge_sw, dst_edge_sw, round, dst_name, flow_src_ip, flow_dst_ip);
    end
