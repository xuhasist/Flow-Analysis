...
k = 4;
hostNum = 1753;
[sw_number, srcNode, dstNode, srcInf, dstInf, g] = createFatTreeTopo(k, hostNum);

%{
% AS_topo
hostNum = 1753;
[sw_number, srcNode, dstNode, srcInf, dstInf, g, asNum, nodeT] = createAsTopo(hostNum);
%}

[link_if, host_ip, sw_struct, link, link_struct, flow_table, hierarchy_cluster] = setVariables(g, srcNode, dstNode, srcInf, dstInf, sw_number, hostNum);
...
for i = 1:size(flow_table, 1)
    i    
    [src_name, dst_name, flow_start_datetime, flow_end_datetime, flow_start_strtime, flow_end_strtime, rate, flow, flow_table, flow_entry] = setFlowInfo(link_bwd_unit, host_ip, flow_table, i);
    
    group = flow_table{i, 'group'};
...
flow_final_path_c = flow_final_path;
link_struct_c = link_struct;
final_network_throuput_c = final_network_throuput;
